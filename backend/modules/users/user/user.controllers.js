...
const User = {
    ...
    async findById(id){
        return UserModel.findOne({ _id: id, is_archived: false}).select('-password')
    },
        ...
        try {
           ...
           if(user){
               if( await bcrypt.compare(password, user.password)){ 
                    ...
                    const token = jwt.sign (
                        { user_id: user._id, email, role: user.role}, process.env.TOKEN_KEY
                    );
    ...
    async validateToken(token) {
        ...
        const user = await UserModel.findOne({ email:decoded.email, is_archived: false});
